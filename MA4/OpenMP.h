...
#pragma omp parallel
    {
        int tid = omp_get_thread_num();
        int local_max = 0; //initialize local max

        for (int i = tid * workload; i < (tid + 1) * workload; ++i)
        {
            if (data[i] > local_max)
            {
                local_max = data[i]; //update local max if current value is greater
            }
        }

        if (local_max > max)
        {
            max = local_max; //replace global max with local max if local max is more max
        }
